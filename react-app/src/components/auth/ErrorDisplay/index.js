...
function ErrorDisplay({ errors, id }) {
  ...
  for (let field of splashInputs) {
    if (field.id === "sign-up-email") {
      errors.includes("email : Please enter an email") ||
    ...
    //   errors.includes(
    //     "profile_pic : Please use a valid image URL (https://ex.jpg/jpeg/png)"
    //   )
    //     ? field.classList.add("error-border")
    //     : field.classList.remove("error-border");
    // }
      ...
      for (let form of node.childNodes.values()) {
        if (form.nodeName === "INPUT") {
          ...
          if (formPH === "Server Image") {
            errors.includes(
              "preview_img : Please use a valid image URL (https://ex.jpg/jpeg/png)"
            )
              ? form.classList.add("error-border")
              : form.classList.remove("error-border");
          }
